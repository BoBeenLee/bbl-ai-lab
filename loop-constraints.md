...
## Paths
- The binding denylist is `docs/safety.md`. Read it every run.
- Never edit .env, .env.*, .dev.vars, worker/wrangler.toml
- Never edit worker/src/flows.ts (flow manifest) without human approval
- Never edit .github/workflows/**, ops/repos.md, skills-lock.json, .agents/skills/**
- Never touch .claude/worktrees/** or the untracked operator repo clones

## Code
- Always run tests before proposing a fix
- For worker or flow changes, `cd worker && npm run typecheck` must pass
- For shell script changes, `bash -n scripts/<file>.sh` must pass
- Never use squash or rebase merge; this repo uses regular merge only
