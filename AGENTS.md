...
- The same manifest also covers project repos, which live under `projects/` rather than `ops/`. `path` is resolved from the hub root, so it need not start with `ops/`. An operator repo owns operations for a remote host; a project repo owns a body of work that is not a Telegram/Actions flow — `projects/games` owns game-related agent skills, `projects/travel` owns trip planning, and `projects/finance` owns a personal-finance knowledge base. All install the same way and none is tracked by the hub.
- A manifest repo may be private. `games`, `travel`, and `finance` are, because they hold personal data. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
