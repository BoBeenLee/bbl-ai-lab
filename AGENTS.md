...
- A manifest repo may be private. `games`, `travel`, `finance`, `music`, `shopping`, and `hiking` are. The first three hold personal data outright; the rest become personal data as they accumulate — generated songs and taste for `music`, purchase history and sizes for `shopping`, GPS tracks and fitness numbers for `hiking`. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
