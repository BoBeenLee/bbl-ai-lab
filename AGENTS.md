...
- A manifest repo may be private. `games`, `travel`, `finance`, `music`, `shopping`, `hiking`, `cad`, `voice-agent`, and `promo-agent` are. The first three hold personal data outright; the rest become personal data as they accumulate — generated songs and taste for `music`, purchase history and sizes for `shopping`, GPS tracks and fitness numbers for `hiking`, home measurements and fabrication history for `cad`, call recordings and transcripts for `voice-agent`, and third-party posts, handles and neighbourhoods scraped from communities for `promo-agent`. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
