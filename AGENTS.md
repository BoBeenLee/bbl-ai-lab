...
- The same manifest also covers project repos, which live under `projects/` rather than `ops/`. `path` is resolved from the hub root, so it need not start with `ops/`. An operator repo owns operations for a remote host; a project repo owns a body of work that is not a Telegram/Actions flow — `projects/games` owns game-related agent skills, and `projects/travel` owns trip planning. Both install the same way and neither is tracked by the hub.
- A manifest repo may be private. `games` and `travel` are, because they hold personal data. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
