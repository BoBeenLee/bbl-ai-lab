...
- The same manifest also covers project repos, which live under `projects/` rather than `ops/`. `path` is resolved from the hub root, so it need not start with `ops/`. An operator repo owns operations for a remote host; a project repo owns a body of work that is not a Telegram/Actions flow — `projects/games` owns game-related agent skills, `projects/travel` owns trip planning, `projects/finance` owns a personal-finance knowledge base, `projects/music` owns AI music production knowledge, `projects/shopping` owns real-purchase decision workflows, `projects/hiking` owns hiking knowledge and per-outing records, `projects/cad` owns FreeCAD knowledge and per-design records (3D-printable parts, Python-generated models, furniture and interior layouts), `projects/voice-agent` owns voice-agent knowledge and per-agent records, gated on staying free, `projects/promo-agent` owns promotion-channel knowledge and per-campaign records, gated on measuring a platform's surface before automating it, and `projects/kakao-agent` owns a KakaoTalk bot on the Mac that runs a selectable local agent CLI (Claude Code, Codex, Gemini, opencode) per turn through Iris on an Android emulator, on its own KakaoTalk account next to the DGX bot. All install the same way and none is tracked by the hub. Project repos register themselves: `repo-sync.sh` scans `projects/*/`, and any clone missing from the manifest is appended from its own `origin` URL and default branch, so neither `ops/repos.md` nor `.gitignore` (a `/projects/*/` glob) is edited by hand. A directory with no `origin` is warned about and skipped. Do not hand-edit `ops/repos.md` for a `projects/` entry — add the remote and run the sync.
- A third kind of manifest entry is an upstream tool repo, owned by its upstream author rather than by me. `ops/openmontage` is the only one: an AGPL-3.0 video production pipeline that calls `ops/remote-comfyui`'s DGX ComfyUI as one provider inside its `assets` stage. Its `.gitignore` line is added by hand like an operator repo's, `repo-sync.sh` clones but does not install (`make setup` inside the repo does), and local edits have nowhere to push until it is forked. See `knowledge/concepts/operator-workspace-repo.md`.
- A manifest repo may be private. `games`, `travel`, `finance`, `music`, `shopping`, `hiking`, `cad`, `voice-agent`, `promo-agent`, and `kakao-agent` are. The first three hold personal data outright; the rest become personal data as they accumulate — generated songs and taste for `music`, purchase history and sizes for `shopping`, GPS tracks and fitness numbers for `hiking`, home measurements and fabrication history for `cad`, call recordings and transcripts for `voice-agent`, third-party posts, handles and neighbourhoods scraped from communities for `promo-agent`, and the owner's and bot's account ids and room ids for `kakao-agent`. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
