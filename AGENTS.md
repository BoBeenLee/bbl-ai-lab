...
- A manifest repo may be private. `games`, `travel`, `finance`, `music`, `shopping`, `hiking`, and `cad` are. The first three hold personal data outright; the rest become personal data as they accumulate — generated songs and taste for `music`, purchase history and sizes for `shopping`, GPS tracks and fitness numbers for `hiking`, home measurements and fabrication history for `cad`. Private entries need a `gh auth` session or `GIT_TOKEN=<pat>` to clone.
