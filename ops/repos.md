...
`games`, `travel`, `finance`, `music`, `shopping`, `hiking`, `cad`, `voice-agent`, `promo-agent`는 private다. 앞의 셋은 계정 로스터,
...
통화 녹음·전사·목소리가, `promo-agent`는 커뮤니티에서 수집한 제3자의 글·닉네임·동네가 쌓이면 개인 데이터가
되기 때문이다.
